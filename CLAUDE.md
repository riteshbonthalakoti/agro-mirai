...
**Modules 01–26 are complete. The backend/production-hardening push (Modules 16–23, `docs/ROADMAP_PRODUCTION.md`) that took the project from "capstone MVP" toward "production grade" is finished. Module 26 (2026-09-11) is a BREAKING MIGRATION, not a feature add: `/v2` auth moved from Module 19's custom bcrypt+signed-session model to real Supabase Auth — the mobile app now talks to Supabase Auth directly (direct-SDK-from-mobile, Flask is a pure JWT-verifying resource server), `/v2/auth/register|login|logout` no longer exist, and `Farmer.id` is now the Supabase `auth.users.id` itself. See `decisions/0022-supabase-auth-migration.md`. `/v1` (Module 19's shared-API_KEY surface) is untouched. Next: the frontend phase (PRD, then Antigravity — React Native + Expo mobile app, Next.js landing page, Module 19's Jinja2 admin kept as-is) — not yet started, and its mobile auth screens now need to target Supabase Auth directly per Module 26's contract, not the removed `/v2/auth/*` endpoints.** Module 16 (Reliability & CI Hardening — workstream A) landed 2026-08-29: GitHub Actions CI (`.github/workflows/ci.yml`, gates on `pytest --ignore=tests/voice` + `check_specs.py`, non-blocking `ruff` lint), Sentry error monitoring (`sentry-sdk[flask]`, no-op without `SENTRY_DSN`), request-id structured logging, an input-validation audit of `POST /fields`/`POST /feedback` (real gaps found and fixed — see `src/agro_mirai/api/validation.py`), `tools/backup_supabase.py` + `docs/BACKUPS.md`, and per-API-key rate limiting (Flask-Limiter, 60/min default).
