...
## Update 2: weather sources on Render (found live)
Open-Meteo's forecast API returns HTTP 429 from Render's shared IP, and the old fallback (OpenWeatherMap "current" only) has no history or forecast, so production never got the v2 data. Now: Open-Meteo first; on failure NASA POWER (30 days observed, ~3 days behind) + MET Norway (9-day forecast) + OpenWeatherMap current, fetched in parallel (~2 s). The newest missing days are filled by repeating the last known day with no rain (max 5 days, else the balance refuses and the old 7-day shortcut is used) and the advice says so. 12-month rainfall now comes from NASA POWER first (597 mm vs Open-Meteo 591 at Bellary, 1.5 s vs 7.7 s).
Also fixed in the same pass: saving weather one row at a time cost 3 Supabase round trips per row (35 s to create a field, and a dropped connection left half the history saved). Now one bulk upsert (`save_weather_readings`, additive on the `DataStore` interface), days already stored are skipped, weather and soil run side by side and soil gets a 5 s wait then finishes in the background ("gathering"). Field creation 35 s -> 7.7 s in production.
Limits: forecast rows from each refresh are kept (rows are not deleted), so a stale forecast for the same day can exist; only history is de-duplicated on write. MET Norway asks clients to identify themselves (User-Agent set). Supabase bulk path has no local contract test (no credentials here); it was confirmed by the production run.
