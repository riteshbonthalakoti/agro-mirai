"""Integration test: fixture -> FeatureBuilder -> DiseaseRiskModel.
...
weather+soil but no NDVI — both should still produce a valid alert
since NDVI absence only zeroes that one scoring component
...
def _alert_for_first_field(fixture_name: str, as_of: date):
...
def test_full_chain_produces_schema_valid_alert(fixture_name):
    rec, field = _alert_for_first_field(fixture_name, as_of=date(2026, 8, 24))
