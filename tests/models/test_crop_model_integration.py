"""Integration test: fixture -> FeatureBuilder -> mapping -> CropRecommendationModel.
...
weather+soil but no NDVI — both should still produce a valid
recommendation since NDVI is not used by this model version
(decisions/0007-crop-model-feature-mapping.md).
"""
...
def _recommend_for_first_field(fixture_name: str, as_of: date):
...
def _to_fixture_record(rec) -> dict:
    """Serializes like a JSON fixture record: ISO timestamp string,
...
def test_full_chain_produces_schema_valid_recommendation(fixture_name):
    rec, field = _recommend_for_first_field(fixture_name, as_of=date(2026, 8, 24))
